...
const CustomItemRenderer: React.FC<CustomItemRendererProps> = ({ item, itemContext, getItemProps }) => {
  ...
  const elapsedDuration = currentTime.isAfter(startTime) ? currentTime.diff(startTime) : 0
    ...
    <TooltipProvider>
      ...
      >
        <div className="flex justify-between items-center mb-2">
          <span className="font-semibold text-sm truncate mr-2">{item.title}</span>
          <Tooltip>
            <TooltipTrigger asChild>
              <Badge variant="outline" className={`text-xs ${getStatusColor()} text-white`}>
                {getStatusText()}
              </Badge>
            </TooltipTrigger>
            <TooltipContent>
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2 text-xs text-white">
            ...
            <span>{durationInHours.toFixed(1)}h</span>
          </div>
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="relative w-16 h-8">
                <svg className="w-full h-full" viewBox="0 0 100 50">
                  ...
                  />
                  <text x="50" y="55" textAnchor="middle" fontSize="20" fill="#1f2937">
                    {progress.toFixed(0)}%
                  </text>
                </svg>
              </div>
        ...
        <div className="flex items-center gap-1 mt-2 text-black">
          ...
          <Button
            size="sm"
            variant="outline"
            onClick={() => handleButtonClick(item.id, "Delete")}
